...
class BookApiFilter(filters.FilterSet):
    ...
    author = filters.CharFilter(field_name='authors', method='filter_by_author', label='author')
    ...
    def filter_by_author(queryset, name, value):
        if " " in value:
            ...
            fullname = queryset.filter(authors__first_name=first_name).filter(authors__last_name=last_name)
        else:
            return queryset.filter(authors__first_name=value.title()) or \
                   queryset.filter(authors__last_name=value.title())
